...
def channel_label(index, channel_id):
    ...
    try:
        ...
        if title:
            CHANNEL_TITLE_CACHE[channel_id] = title
            return title
    except Exception:
        pass
    return f"Kanal {index + 1}"
    ...
    for index, channel_id in enumerate(REQUIRED_CHANNEL_IDS):
        try:
            ...
            if member.status not in SUBSCRIBED_STATUSES:
                missing.append((index, channel_id))
        except Exception:
            missing.append((index, channel_id))
    return missing
...
def ensure_call_subscribed(call):
    if ensure_subscribed(call.message.chat.id, call.from_user.id):
        return True
    bot.answer_callback_query(call.id, "Avval kanallarga obuna bo'ling.")
    return False
...
def answer_handler(call):
    if not ensure_call_subscribed(call):
        return
...
def start_handler(message):
    if not ensure_subscribed(message.chat.id, message.from_user.id):
        return
...
def approve_price(call):
    ...
    try:
        ...
        if images:
            ...
            bot.send_message(CHANNEL_ID[0], caption)
